...
const cognitoAuthProvider = {
  ...
  jwtToken: '',
  cognitoUser: {},
  tryLoginActiveSession(callback: Function) {
      ...
      cognitoUser.getSession(function (err: any, result: any) {
        if (result) {
          ...
          AWS.config.credentials = new AWS.CognitoIdentityCredentials({
            IdentityPoolId: 'us-west-1:7b749e82-7231-4f83-8b42-bce90babaa83', // your identity pool id here
            Logins: {
      ...
      (AWS.config.credentials as AWS.CognitoIdentityCredentials).refresh(
        (error: any) => {
          if (error) {
            ...
            callback(cognitoUser?.getUsername(), cognitoAuthProvider.jwtToken);
          }
        },
      );
    }
  ...
  handleNewPassword(
    ...
    (
      cognitoAuthProvider.cognitoUser as CognitoUser
    ).completeNewPasswordChallenge(
      newPassword,
      sessionUserAttributes,
      callback(
        (cognitoAuthProvider.cognitoUser as CognitoUser).getUsername(),
        cognitoAuthProvider.jwtToken,
      ),
    );
  },
};
